...
void przygotowanie_gry()
{
    ...
    for(int x=0;x<(int)haslo.length();x++)
    {
        zakodowane_haslo+="_";
    }
...
void write_message(const std::string& message,const std::string& type, int fd)
{
    char test[255];
    Message mess;
...
void send_to_all_active(const std::string& message,const std::string& type)
{
    for(int j=0; j<100;j++)
    {
...
int main(int argc, char ** argv){
    ...
    sockaddr_in serverAddr{.sin_family=AF_INET, .sin_port=htons((short)port), .sin_addr={INADDR_ANY},.sin_zero={0}}; //sin_zero jak nie ma to warnig przy cmake(kompil)
        ...
        for(int i=0;i<wait;i++)
        {
            ...
            if(events[i].events & EPOLLIN)
            {
                ...
                if(count > 0){ 
                    ...
                    {   std::string wisielec_str = std::to_string(0);
                        write_message(wisielec_str,"GAME",events[i].data.fd);
                    }
                    if(mess_cli.type == "WISIELCE")
                    {   
                        std::string str;
                        for(int j=0; j<100;j++)
                        {
                            if(clients[j].acctive==1)
                            {
                                str+= clients[j].name+"-"+std::to_string(clients[j].trys)+"=";
                            }
                        }
                        str+=";";
                        write_message(str,"WISIELCE",events[i].data.fd);
                    }
                    if(mess_cli.type == "PYTLIT")
                    {
                        ...
                        if(clients[events[i].data.fd].trys ==9)
                        {
                            end_scorse++;
                            std::cout<<end_scorse<<"\n";
                        ...
                        {   
                            if(winner==0)
                            {
                                end_scorse++;
                                clients[events[i].data.fd].points++;
                                std::string str=std::to_string(clients[events[i].data.fd].trys)+clients[events[i].data.fd].odgadywane_haslo;
                                write_message(str,"WIN",events[i].data.fd);
                                winner=events[i].data.fd;
                            }
                            else{
                                end_scorse++;
                                std::string end_not_first;
                                end_not_first = std::to_string(clients[events[i].data.fd].trys)+clients[events[i].data.fd].odgadywane_haslo + " Zgadles, ale nie pierwszy :(";
                                write_message(end_not_first,"WAIT",events[i].data.fd);
                            }
                        }
                        else{

                            std::string wisielec_str = std::to_string(clients[events[i].data.fd].trys)+clients[events[i].data.fd].odgadywane_haslo;
                            write_message(wisielec_str,"GAME",events[i].data.fd);
                        }
                    }
